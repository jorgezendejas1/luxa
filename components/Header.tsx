...
const HeartIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 016.364 0L12 7.664l1.318-1.346a4.5 4.5 0 116.364 6.364L12 21l-7.682-7.682a4.5 4.5 0 010-6.364z" />
    </svg>
);
...
const Header: React.FC<HeaderProps> = ({ onSelectCategory, onSearch, onNavigate }) => {
        ...
        <header className="bg-white shadow-md sticky top-0 z-50">
            ...
            <div className="container mx-auto px-4">
                <div className="flex justify-between items-center py-4">
                    ...
                    <div className="flex items-center space-x-4">
                        <CurrencyConverter />
                        <button onClick={() => onNavigate('wishlist')} className="relative text-gray-600 hover:text-pink-500" aria-label="Wishlist">
                            <HeartIcon />
                            {favoritesCount > 0 && <span className="absolute -top-2 -right-2 bg-pink-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center animate-popIn">{favoritesCount}</span>}
                        </button>
                        <button onClick={() => onNavigate('cart')} className="relative text-gray-600 hover:text-pink-500" aria-label="Shopping cart">
                            <CartIcon />
                            {cartCount > 0 && <span className="absolute -top-2 -right-2 bg-pink-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center animate-popIn">{cartCount}</span>}
                        </button>
                    </div>
                </div>
            </div>
